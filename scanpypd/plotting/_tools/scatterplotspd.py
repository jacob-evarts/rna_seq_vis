...
def _wraps_plot_scatter(wrapper):
    ...
    params = inspect.signature(embedding_data).parameters.copy()
...
def umap_data(adata, **kwargs) -> Union[Axes, List[Axes], None]:
    """\
    ...
    tl.umap
    """
    return embedding_data(adata, 'umap', **kwargs)
...
def tsne(adata, **kwargs) -> Union[Axes, List[Axes], None]:
    ...
    tl.tsne
    """
    return embedding_data(adata, 'tsne', **kwargs)
...
def diffmap(adata, **kwargs) -> Union[Axes, List[Axes], None]:
    ...
    tl.diffmap
    """
    return embedding_data(adata, 'diffmap', **kwargs)
...
def draw_graph(
    ...
    return embedding_data(adata, basis, **kwargs)
    ...
    **kwargs,
    ...
    if not annotate_var_explained:
        ...
        if return_fig is True:
            ...
            fig = embedding_data(adata, 'pca', return_fig=return_fig, **kwargs)
            for ax in fig.axes:
            ...
            axs = embedding_data(adata, 'pca', show=False, save=False, **kwargs)
